...
class User implements IModel
{
    ...
    protected function includeUserRole(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        ...
        if (!empty($foreignKeyIds)) {
            ...
            foreach ($items as &$item) {
                ...
                $relatedRecords = $tableModelName->findMany(['where' => [$primaryKey => $item[$foreignKey]], 'select' => $includeSelectedFields], $format);
    ...
    public function findUnique(array $criteria, bool $format = false): array | object
    {
        ...
        if (isset($where['email'])) {
            ...
            $validatedValue = Validator::string($where['email']);
